...
def chat_request_response(prompt, chat_history):
    # Concatenate the chat history with the new prompt
    full_prompt = "\n\n".join(chat_history) + "\n\n" + prompt
    ...
    response = openai.Completion.create(
        engine=MODEL,
        prompt=full_prompt,
        temperature=0.7,  # You can adjust this value to control the output randomness
        max_tokens=1000,
        n=1,
        stop=None,
        frequency_penalty=0,
...
response = chat_request_response(first_prompt, chat_history)
print(response)
chat_history.append(response)

# Call the function to generate subsequent responses
for phase in phases:
    prompt = f"Write a paragraph in the '{phase}' phase of the {random_arc} arc style, continuing the story from the previous paragraphs."
    response = chat_request_response(prompt, chat_history)
    print(response)
    chat_history.append(response)
